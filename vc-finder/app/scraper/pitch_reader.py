...
HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; VCFinderBot/0.1)"
}
...
SUMMARY_PROMPT = """あなたはスタートアップのピッチ資料を読み解くアナリストです。
以下はあるスタートアップのピッチ資料から抽出したテキストです。
このスタートアップの「自社事業情報」を、VC/CVC探索に使えるように日本語で簡潔に構造化してください。

次の見出しで、わかる範囲だけを箇条書きでまとめてください（資料に無い項目は省略）:
- 会社名 / サービス名
- 事業内容（何を提供しているか、1〜2文）
- 顧客・ターゲット市場
- 保有する事業アセット（設備・拠点・技術・データ・顧客基盤・チャネル等。VCの事業会社が魅力を感じうるもの）
- 調達ステージ / 事業フェーズ
- 業界・ドメイン
