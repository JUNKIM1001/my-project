...
// 静的サイト用に自己完結化: 使用するのは非三角形モードの変換と、スキン付きノードの複製のみ。
// 本アプリの GLB は TRIANGLES / スキン無しなので、未対応時は明示的にエラーにする。
function toTrianglesDrawMode() { throw new Error('jam-lab: TRIANGLE_STRIP/FAN の GLB は未対応です'); }
function clone(object) { return object.clone(true); }
