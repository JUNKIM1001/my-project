...
const LEVEL_01 = {
  ...
  goals: [
    'ブレーキを 1〜2 秒踏み、後ろへ進む波の速度を計測する（★1）',
    `1 回の踏み込みを ${WAVE_SPREAD_CARS} 台以上に届かせる（★2）`,
    'クイズに正解する（★3）',
  ],
  lesson: {
...
const LEVEL_02 = {
  ...
  goals: [
    '踏み込みを 2 回以上記録する（★1）',
    `短い踏み込みと長い踏み込みを両方試す（★2）`,
    `長いほうの遅れを短いほうの ${LENGTH_CONTRAST_RATIO} 倍以上にする（★3）`,
  ],
  lesson: {
...
const LEVEL_03 = {
  ...
  goals: [
    '踏み込みを 2 回以上記録する（★1）',
    '2 通り以上の台数で踏む（★2）',
    `混んだ側の影響を、すいた側の ${DENSITY_CONTRAST_RATIO} 倍以上にする（★3）`,
  ],
  lesson: {
...
const LEVEL_04 = {
  ...
  goals: [
    '踏み込みを 1 回以上記録する（★1）',
    `τ を変えて比べる、または 1 回の踏み込みを ${REACTION_AMPLIFIED_CARS} 台以上に広げる（★2）`,
    'クイズに正解する（★3）',
  ],
  lesson: {
...
const LEVEL_05 = {
  ...
  goals: [
    '実験を最後まで行う（★1）',
    `ブレーキの合計を ${SAG_MAX_BRAKE_SEC.toFixed(1)} 秒以下に抑える（★2）`,
    `車列全体の遅れを ${SAG_MAX_LOSS_SEC} 秒未満に抑える（★3）`,
  ],
  lesson: {
